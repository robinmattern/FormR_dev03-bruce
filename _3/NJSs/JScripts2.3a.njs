...
//          --------------  =  ----------------------------------------------------
  ...
  function  trace( aMsg, bShow ) { 
        ...
        if (String(aMsg).match( /setProj.*/i )) { TheProject = bShow; return }                                 // .(10301.01.2 RAM Set for parsing functino stall stack)
        if (String(aMsg).match( /setShow.*/i )) { bQuiet     = bShow == false; return }                        // .(10303.01.1 RAM).(10303.12.1 RAM Gotta use String())
...
//     let  mStack =  aError.stack.split( '\n' )
       let  mStack =  aError.stack.split( '\n' ).filter( aLine => aLine.match( TheProject ) != null )
        if (mStack.length == 0) { console.log( " ** trace() project name is not set."); return }               // .(.(10301.01.2)
...
//          --------------  =  ----------------------------------------------------
  ...
  function  doTest( nTest, aFilename ) { var nDoTests1 
       var  bTesting  =   (aFilename ? aFilename : __filename) == process.mainModule.filename                   // .(10301.04.6 RAM if parent script is not called by another)
        if (bTesting && typeof( nDoTests ) == 'undefined') { nDoTests1 = nDoTests = ',,' }                      //                  ie we may be performing a test  
...
//          --------------  =  ----------------------------------------------------
        ...
        if (typeof(__dirname) == 'undefined' ) {                                                                // .(10301.11.1 RAM __dirname is not in v10.12+)
...
//          --------------  =  ----------------------------------------------------
  ...
  function  shoRoutes( pApp, aShoEm ) {
        if (String(aShoEm) != 'true') { return }                                                                // || ! require( 'fs' ).existsSync( '../../server/node_modules/express-list-endpoints' ) ) { return }  
 try { var  listRoutes  =  require( '../../server/node_modules/express-list-endpoints' ) } catch(e) {           // .(10224.02.1 RAM Bruce doesn't have it loaded).(10301.14.1 RAM Moving it here requires a different path)   
            ...
            console.log( "\n" + listRoutes( pApp ).map( pRoute => fmtRoute( pRoute ) ).join( "\n" ) )
...
//          --------------  =  ----------------------------------------------------
  ...
  function  setEnv( aDirName ) { // trace()                                             // .(01012.01.2 Beg).(10303.02.1 RAM Moved to JScript.njs)
       var  aApp =  aDirName.split( /[\\\/]/ ).splice( -4, 4 ).join( '/' )              // .(10303.02.3 RAM Let get it from __DirName)
            process.env.APP_NAME = aApp                                                 // .(10303.02.1 RAM Needed AppName here. Save it!)
            process.env.DIRNAME  = aDirName                                             // .(10303.02.4 RAM Save it too!)
       if (!process.env.PORT) {
       var  aAppDir = aApp.split( '/' ).splice( -2, 2 ).join( '/' )                     // .(10303.02.5)
...
//          } )( 'getArg parse mIota take cnum say sho now setPage logIt fmtTS fmtDate fmtAmt fmtObj Alert trace doTest' )   // .(10228.09.2)
            } )( 'getArg parse mIota take cnum say sho now setPage logIt fmtTS fmtDate fmtAmt fmtObj Alert trace doTest __dirname __filename' )   // .(10228.09.2)
