...
//                     : `${BRANCH_HOME}/${ APP_DIR.split( '/' )[0] }/node_modules/`                            // .(10305.02.1 RAM Gotta get server or client folder name)
       var  nDepth     =    APP_HOME.match( /FRAs/ ) ? 2 : 1                                                    // .(10328.05.5 RAM Jeese Louise to the 4th!)
...
//          aDirName =  aDirName.replace( /[\\/](api|src|_4).*/i, '')                   //#.(10317.03.1 RAM Peel back to before /api or /src).(10318.02.1 RAM or _4)
       ...
       var  nDepth   =  aDirName.match( /FRAs/ ) ? 3 : 2                                // .(10328.05.1 RAM Jeese Louise cubed!)
...
//      if (aAppName.match( /server.+([\\/]app)*/ )) {                                  // .(10318.02.3 RAM Jeese Louise)
        if (aAppName.match( /server.+([\\/]app)*/ ) || nDepth == 3) {                   // .(10318.02.3 RAM Jeese Louise).(10328.05.3 RAM Added FRAs. Jeese Louise is right)
...
//          console.log( `  * App, ${aAppName}, is Not an App.` )                       // .(10317.03.3)
            ...
            FORMRs_4 = `${ aDirName.replace( /[\\/]app[0-9]+[sc][0-9]*/i, '' ) }/_4/FRSs/` // .(10317.01.1 RAM Server's Home if dynamically assigned).(10327.02.1 RAM Allow for trailing no: e.g. app08s1)
                      if ( aDirName.match(   /_4/ ) ) {                                    // .(10331.02.1 RAM Will we ever get it right)
            FORMRs_4 = `${ aDirName.replace( /_4[\\\/]FRAs/, '_4/FRSs/' ) }` }             // .(10331.02.2)
            }
//          --------------  =  ----------------------------------------------------
...
var pReq =  sndAPI(   aMethod, aURL, pData, aToken, xNext ) 
            onSubmit( pReq, xNext )
            xAJAX2(   pReq ).then( onSuccess ).catch( onFailure )
...
//      -----------  =   ----------------------------
 ...
 function  onSuccess( pResponse ) {
        ...
        var mRecs = pResponse.body.length ? mRecs : [ pResponse.body ]      
...
// ----- ----------  =    ---------------------------------------------------------
  ...
  function  onSubmit( pReq, xNext ) {
        ...
        if (pReq.json) {                                                                // .(10402.04.5 RAM Not sure if this is the best way)
            console.log( " Request.data:"     + fmtObj( pReq.json,    6 ) ); 
        } else {                                                                        // .(10402.04.6 Beg)
        if (pReq.json) {
